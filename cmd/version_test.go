...
func TestVersionCheckListOfAgents(t *testing.T) {
	for _, testCase := range []struct {
	...
	}{
		{
			...
			expStatus: statusOK,
			expValue:  "10.0.6.233",
			version:   `[ "adminrouter--25519a43df187f2c08d854373802dc4dcf12d010", "avro-cpp--e9b81a0108708cd2c298748cadbc7798a2beca20"]`,
		},
	} {
		mockCLICfg := &CLIConfigFlags{
			NodeIPStr: "127.0.0.1",
		...
		versionurlopt.path = "/pkgpanda/active/"
		...
		if version[0] != "adminrouter--25519a43df187f2c08d854373802dc4dcf12d010" {
			t.Fatalf("Getting nonsense %v, not the correct value", version)
		}
